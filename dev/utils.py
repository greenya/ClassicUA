...
def write_lua_item_text_file(path, filename, item_texts):
    Path(path).mkdir(parents=True, exist_ok=True)
    with open(f'{path}/{filename}.lua', mode='w', encoding='utf-8', newline='\n') as f:
        write_lua_autogenerated_file_comment(f)
        f.write('local _, addonTable = ...\n\n')
        f.write('local item_text = { -- [id] = { page1, page2 (optional), page3 (optional), ... }\n')

        for id in item_texts:
            pages, hint = item_texts[id]
        ...
        f.write('}\n\n')
        write_lua_assign_or_copy_table_code(f, 'item_text')

def write_lua_object_text_file(path, filename, objects):
    Path(path).mkdir(parents=True, exist_ok=True)
    with open(f'{path}/{filename}.lua', mode='w', encoding='utf-8', newline='\n') as f:
        write_lua_autogenerated_file_comment(f)
        f.write('local _, addonTable = ...\n\n')
        f.write('local object_text = { -- [name] = { page1, page2 (optional), ... }, or a list of namesakes { { pages..., h1=hash of the english page 1 }, ... }\n')

        def write_pages(pages, indent=''):
            for page in pages:
                f.write(f'{indent}[===[{page}]===],\n')
        ...
        for name in objects:
            ...
            if isinstance(entry, list):
                f.write(f'[{key}] = ' + '{ -- namesakes, told apart by the first page\n')
                for pages, hint, h1 in entry:
                    f.write('{' + f' -- {hint}\n')
                    write_pages(pages)
                    f.write(f'h1={h1} ' + '},\n')
                f.write('},\n')
                ...
                pages, hint = entry
                f.write(f'[{key}] = ' + '{' + f' -- {hint}\n')
                write_pages(pages)
                f.write('},\n')

        f.write('}\n\n')
        write_lua_assign_or_copy_table_code(f, 'object_text')
...
def write_lua_talent_tree_file(path, filename, talents):
    Path(path).mkdir(parents=True, exist_ok=True)
    with open(f'{path}/{filename}.lua', mode='w', encoding='utf-8', newline='\n') as f:
        write_lua_autogenerated_file_comment(f)
        f.write('local _, addonTable = ...\n\n')
        f.write('addonTable.talent_tree = { -- [talent id] = { spell id of rank 1, rank 2, ... }\n')

        group = None
        for t in talents:
            if (t['class'], t['tab']) != group:
                group = (t['class'], t['tab'])
                f.write(f'\n-- {t["class"]}: {t["tab"]}\n')

            f.write(f'[{t["id"]}] = ' + '{ ' + ', '.join(str(r) for r in t['ranks']) + ' },\n')
