...
[24858] = { "Подоба місячного сованя", "Надає друїдові подоби місячного сованя, збільшуючи броню від предметів на {1}%. Також збільшує ймовірність критичного удару заклять учасників групи в радіусі {2} м на {3}%. У подобі місячного сованя можна використовувати лише закляття спеціализації \"Баланс\".\n\nПри активації, звільняє заклинача від ефектів поліморфу та ефектів перешкоди руху.#items is increased by {1}#within {2} y#chance increased by {3}", "Невразливість до ефектів поліморфу. Броня від предметів збільшена {1}%.#by {1}" }, -- Moonkin Form
...
-- Pounce
[9005] = { "Стрибок", "Враз стрибнути, приголомшити ціль на {1} с та завдайта {2} пошкодження протягом {3} с. Потрібно зачаїтися та перебувати позаду цілі. Збільшує довжину комбінації на {4} прийом.#for {1} sec#causing {2} damage over {3} sec#{4} combo", "Приголомшений." }, -- Pounce (Rank 1)
