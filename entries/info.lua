...
changelog = [===[
Версія 6.8 / 19.09.26
— Додано переклад 375 завдань TBC (Акіндун, Долина Тінистого Місяця, Скеттіс, Шаттрат, підземелля і рейди)
— Додано переклад 6 книжок, 675 персонажів, 28 заклять, 564 предметів, 32 об'єктів, 17 локацій, 137 повідомлення чату та 44 пліток
— Виправлено помилку з бульбашками чатів, які інколи не перекладалися

Версія 6.7 / 25.08.26
— Додано сторінку розширених налаштувань перекладу
— Додано показ накопичених даних розробки для копіювання з гри
— Додано переклад назв предметів у вікнах завдань
— Додано переклад імені персонажа в заголовку вікон завдань і розмови
— Додано перемикач мови у вікнах розмови
— Оновлено переклад чату
— Виправлено розмір шрифту імен у плаваючих фреймах персонажів
— Виправлено визначення місця прив'язки каменя повернення

Версія 6.6.2 / 22.07.26
— Оновлено підтримувану версію гри для Classic (1.15.9)

Версія 6.6.1 / 12.07.26
— Виправлено проблеми з відображенням плаваючих текстів бою із заміною стандартних шрифти
— Оновлено підтримувану версію гри для TBC (2.5.6) та Mists (5.5.4)

Версія 6.6 / 30.05.26
— Додано переклад 168 завдань TBC (Низошторм, Лезокраї гори, Шаттрат, Склепи мани)
— Додано переклад 549 персонажів, 935 заклять, 1232 предмета, 72 об'єктів, 109 локацій, 34 повідомлення чату та 9 пліток

Версія 6.5 / 17.01.26
— Додано переклад 128 завдань TBC (Лезокраї гори, Низошторм, Шаттрат)
— Виправлено lua-помилки з роботою клієнта TBC Anniversary
— Оновлено підтримувану версію гри для TBC (2.5.5)

Версія 6.4 / 13.12.25
— Додано переклад 20 завдань TBC (Лезокраї гори)
— Додано переклад 6500+ предметів та 6500+ заклять (TBC, Wrath, Cata, Mists)
— Додано переклад 3 об'єктів, 35+ повідомлень чату та пліток, 40+ рядків інтерфейсу
— Додано підтримку перекладів суфіксів предметів ("of Agility", "of Battle" тощо)
— Додано колонку по Mists у секцію зі статистикою у вікні налаштувань
— Виправлено lua-помилку при відображені підказок талантів Mists
— Виправлено lua-помилку при відображенні мапи
— Виправлено помилку відображення імені гравця в чаті, при відсутності перекладу фрази
— Оновлено підтримувану версію гри для Classic (1.15.8) та Mists (5.5.3)

Версія 6.3 / 12.07.25
— Додано переклад 305 завдань: 19 завдань SOD та 286 завдань TBC (Ліс Тероккар, Наґранд, Шаттрат, Акенайські склепи, Підболоття, Склепи мани).
— Додано переклад 5 книжок, 310 персонажів, 301 предмета, 46 об'єктів, 69 локацій, 132 повідомлень чату та 23 плітки.
— Додано секцію "Завдання" у вікно налаштувань; відображає прогрес перекладу завдань по доповненнях та категоріях.
— Додано підтримку Mists-клієнта гри.

Версія 6.2 / 03.05.25
— Додано переклад 8-ої фази SOD: 73 завдання, 2 книжки, 133 закляття, 280 персонажів, 549 предметів, 11 об'єктів, 41 локація
— Виправлено lua-помилку при спробі записати знайдену помилку з увімкненим режимом розробки

Версія 6.1 / 12.04.25
— Додано переклад реплік чату для 350+ персонажів для Classic та SOD.
— Додано переклад списку реплік (запитів) при спілкуванні з персонажами.
— Перероблено панель зі статистикою (лічильниками), тепер це окрема секція з деталями по доповненнях.
— Виправлено проблеми з відображенням плаваючих текстів бою
— Оновлено підтримувану версію гри для Classic (1.15.7)

Версія 6.0.1 / 15.02.25
— Додано переклад списків активних та поточних завдань при спілкуванні з персонажами.
— Додано перемикач мови для завдань і книжок у верхній правий кут вікна з перекладом.
— Додано налаштування для відключення перекладу плаваючих фреймів.
— Виправлено проблеми з "блокуванням дій аддону" при спробі скористатися деякими функціями контекстних меню цілі (Copy Character Name, Set Focus). Переклад міток цілей рейду тепер відсутній.
— Виправлено проблеми з відображенням деяких текстів після заміни стандартних шрифтів.

Версія 6.0 / 08.02.25
— Додано переклад 38 завдань SOD: Брашпиля, Мерехтливих рівнин, острова Йоджамба, Східних Зачумлених земель, підземель Перевалу Мертвого Вітру та Склепів Каражану
— Додано переклад 13 персонажів Classic, 229 персонажів SOD, 5 книжок SOD, 10 локацій
— Новий підхід до відображення перекладу завдання, книжки та плітки — тепер не має окремого вікна, а налаштування розмірів тексту видалені. Переклад здійснено на рівні даних гри, тому він відображається на місці оригінального тексту. Це помітно не лише в вікні завдання, але й у списку відстеження завдань під мінікартою та на карті світу (для Wrath+).
— Новий підхід до перекладу назв локацій; тепер мапа світу має перекладені випадаючи списки континентів та зон.
— Додано заміну стандартних шрифтів та відповідне налаштування для відключення цієї операції.
— Додано переклад домівки гравця, що відображається в підказках відповідних предметів, наприклад у Каменя повернення.
— Додано переклад деяких рядків інтерфейсу: повідомлення чату, мітки цілі рейду, підказки предметів тощо.

Версія 5.9 / 26.01.25
— Додано переклад 187 завдань SOD: завдання класів, підземель та репутаційні завдання
— Додано переклад 100+ персонажів, 800+ предметів, 2000+ заклять
— Оновлено підтримувану версію гри для Classic (1.15.6) та Cata (4.4.2)

Версія 5.8 / 23.11.24
— Додано переклад 80 завдань TBC: Шаттрат (8%), болота Занґар
— Додано переклад 103 персонажів TBC, 2 локацій, 50 повідомлень чату
— Оновлено підтримувану версію гри для Classic (1.15.5) та Cata (4.4.1)

Версія 5.7 / 26.10.24
— Додано переклад 167 завдань SOD
— Додано переклад 366 персонажів, 107 локацій, 41 повідомлення чату
— Додано переклад 5-ої фази SOD (закляття, предмети, об'єкти)

Версія 5.6.1 / 29.09.24
— Додано кнопку "Скинути" до вікна налаштувань
— Оновлено підтримувану версію гри для Classic (1.15.4)
— SOD: виправлено помилки після оновлення до 1.15.4. Переклад випадаючих списків на карті світу тепер відсутній.
...
Версія 4.1 / 24.09.23
— Додано переклад 242 завдань: Грізний Молот (12%), Західні Зачумлені землі, Зимоключ (17%), Спустошені землі, Стратгольм (29%), Східні Зачумлені землі (43%), Танаріс (64%), Чорноскельна гора, Чорноскельні надра, Чорноскельний шпиль (27%), Некроситет
...
dev_mode_desc = [===[
Ви можете допомогти зі збором неперекладених частин гри: в режимі розробки аддон запам'ятовує неперекладені сутності. Щоб ми отримали ці дані, відправте їх нам власноруч.

Надішліть нам файл даних з теки гри або скопіюйте його вміст, натиснувши на кнопку "Показати дані".

Швидко відкрити теку з грою можна за допомогою Battle.net, для цього справа від кнопки "Play" клацніть на шестерні й виберіть "Show in Explorer".
]===],
